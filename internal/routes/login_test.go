...
func TestLogin(t *testing.T) {
	...
	tests[6].Name = `Has multiple "Content-Type" header`
	tests[6].Request.Method = http.MethodPost
	tests[6].Request.Body = strings.NewReader(fmt.Sprintf(`{"email": %q, "password": %q}`, email, password))
	tests[6].Request.Header = http.Header{
		"Content-Type": []string{"text/plain", "application/json"},
		"Accept":       []string{"application/json"},
	}
	tests[6].Expected.StatusCode = http.StatusOK
	tests[6].Expected.Header = http.Header{
		"Content-Type": []string{"application/json"},
	}

	tests[7].Name = `Has multiple "Accept" header`
	tests[7].Request.Method = http.MethodPost
	tests[7].Request.Body = strings.NewReader(fmt.Sprintf(`{"email": %q, "password": %q}`, email, password))
	tests[7].Request.Header = http.Header{
		"Content-Type": []string{"application/json"},
	...
	tests[9].Name = "No email"
	tests[9].Request.Method = http.MethodPost
	tests[9].Request.Body = strings.NewReader(fmt.Sprintf(`{"password": %q}`, password))
	tests[9].Request.Header = http.Header{
		"Content-Type": []string{"application/json"},
		"Accept":       []string{"application/json"},
	}
	tests[9].Expected.StatusCode = http.StatusUnprocessableEntity
	tests[9].Expected.Header = http.Header{
		"Content-Type": []string{"application/json"},
	}
